...
pub struct DuplexStream {
    ...
    // Track when the last write occurred for accurate interrupt position
...
impl DuplexStream {
    ...
    pub fn new(config: StreamConfig) -> PyResult<Self> {
        ...
        let aec = if config.enable_aec {
            let aec_config = AecConfig {
                ...
                enable_aec: true,
                enable_agc: false,  // Could be made configurable
                enable_ns: false,   // Could be made configurable
            };
    ...
    pub fn start(&mut self) -> PyResult<()> {
        ...
        let output_callback = move |pa::OutputStreamCallbackArgs { buffer, .. }| {
            ...
            if let Some(aec) = &aec_playback {
                if read > 0 {
                    let _ = aec.process_render(&buffer[..read]);
                }
            }
    ...
    pub fn write_output(&self, data: &[u8]) -> PyResult<usize> {
        ...
        // Record when this write occurred
    ...
    pub fn interrupt_output(&self) -> f64 {
        // Calculate how much time has elapsed since the last write
        let interrupt_position = {
            let last_write = self.last_write_time.lock().unwrap();
            if let Some(write_time) = *last_write {
                // Return the time elapsed since the last write
                write_time.elapsed().as_secs_f64()
            } else {
                // No write has occurred yet, return 0
                0.0
            }
        };
        
        // Now request the interrupt
        let reader = self.output_buffer_reader.lock().unwrap();
        reader.request_clear();
        
        // Clear the last write time
        *self.last_write_time.lock().unwrap() = None;
        
        // Return the position within the current audio buffer where interrupt occurred
        interrupt_position
    }
